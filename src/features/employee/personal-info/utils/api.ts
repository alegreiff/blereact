...
const getTiposPersona = async (): Promise<AutocompleteOption[]> => {
  await wait();
  return tiposPersona.map((item) => ({
    label: item.label,
    value: item.value,
  }));
}
...
export { getStates, getCities, getCasos, getTiposPersona, getTipoVinculacion };
